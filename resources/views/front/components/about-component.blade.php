<section class="as_about_wrapper as_padderTop80 as_padderBottom80">
    <div class="container">
        <div class="row">
            <div class="col-lg-6 col-md-12 col-sm-12 col-12">
                <div class="as_about_slider">
                    <div>
                        <div class="as_aboutimg text-right">
                            <img src="{{ asset('assets/images/about-1.webp') }}" alt="{{$settings->site_name}}" class="img-responsive" width="540" height="540" loading="lazy">
                        </div>
                    </div>
                    <div>
                        <div class="as_aboutimg text-right">
                            <img src="{{ asset('assets/images/about-2.webp') }}" alt="{{$settings->site_name}}"
                                class="img-responsive" width="540" height="540" loading="lazy">
                        </div>
                    </div>
                    <div>
                        <div class="as_aboutimg text-right">
                            <img src="{{ asset('assets/images/about-3.webp') }}" alt="{{$settings->site_name}}"
                                class="img-responsive" width="540" height="540" loading="lazy">
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 col-md-12 col-sm-12 col-12">
                <h1 class="as_heading">Astrologer Ankit Shastri Ji </h1>
                <p>Astrology is an ancient sacred science that reveals the powerful connection between planetary
                    movements and human life. Astrologer Ankit Shastri Ji, with 15+ years of experience, has guided
                    thousands of people across India towards a better, peaceful and successful life through the deep
                    knowledge of Jyotish Shastra.</p>
                <p>From love and marriage to business, career, health and family problems — Ankit Shastri Ji provides
                    accurate and honest astrological guidance based on your Janam Kundli and planetary positions. His
                    vast knowledge of Astrology, Numerology, Vastu Shastra and Gemstone Therapy makes him one of the
                    most trusted astrologers in India. Lakhs of satisfied clients from India and abroad trust him for
                    life-changing predictions and real solutions.</p>
                @if (Route::is('about'))
                    <a href="{{ route('contact') }}" class="as_btn" aria-label="Make Appointment">Make Appointment</a>
                @else
                    <a href="{{ route('about') }}" class="as_btn" aria-label="Read More About Us">read more</a>
                @endif


                <div class="as_contact_expert">
                    <span class="as_icon">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                            preserveAspectRatio="xMidYMid" width="20" height="20" viewBox="0 0 20 20">
                            <defs>
                                <style>
                                    .cls-1 {
                                        fill: #fff;
                                        fill-rule: evenodd
                                    }
                                </style>
                            </defs>
                            <path
                                d="M19.797,10.487 C19.668,10.616 19.493,10.689 19.310,10.689 C18.929,10.689 18.620,10.380 18.620,9.999 C18.615,5.241 14.759,1.385 10.000,1.379 C9.619,1.379 9.310,1.070 9.310,0.689 C9.310,0.308 9.619,-0.000 10.000,-0.000 C15.520,0.006 19.993,4.478 19.999,9.999 C19.999,10.183 19.927,10.358 19.797,10.487 ZM15.172,9.999 C15.169,7.144 12.855,4.830 10.000,4.827 C9.619,4.827 9.310,4.518 9.310,4.138 C9.310,3.757 9.619,3.448 10.000,3.448 C13.617,3.452 16.547,6.383 16.551,9.999 C16.551,10.380 16.243,10.689 15.862,10.689 C15.481,10.689 15.172,10.380 15.172,9.999 ZM12.864,14.155 C13.076,14.182 13.288,14.109 13.438,13.957 L14.982,12.413 C15.209,12.186 15.563,12.146 15.835,12.317 L19.655,14.775 C19.955,14.965 20.063,15.350 19.905,15.668 L18.045,19.616 C17.918,19.873 17.645,20.024 17.360,19.995 C15.394,19.789 10.563,18.932 5.815,14.183 C1.067,9.435 0.210,4.604 0.003,2.638 C-0.026,2.352 0.125,2.079 0.382,1.952 L4.331,0.093 C4.649,-0.067 5.036,0.043 5.224,0.344 L7.684,4.164 C7.854,4.436 7.814,4.790 7.586,5.017 L6.042,6.560 C5.890,6.711 5.818,6.924 5.845,7.135 C5.942,7.900 6.373,9.809 8.282,11.718 C10.191,13.627 12.099,14.057 12.864,14.155 Z"
                                class="cls-1" />
                        </svg>
                    </span>
                    <div>
                        <h5 class="as_white">Contact Our Expert Astrologers</h5>
                        @if (!empty($settings->contact_phones[0]))
                            <h1 class="as_orange">{{ $settings->contact_phones[0] }},</h1>
                        @endif
                        @if (!empty($settings->contact_phones[1]))
                            <h1 class="as_orange">{{ $settings->contact_phones[1] }}</h1>
                        @endif
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
