...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.zodiac-signs.multipleDelete') }}" method="POST" id="multiple-delete">
            @csrf
            <div class="row justify-content-center">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h2 class="mb-0 float-start">{{ __('Zodiac Signs') }}</h2>
                            <button type="submit" class="btn btn-danger float-end ms-2">
                                <i class="fa-solid fa-trash"></i> Delete Selected
                            </button>
                            <a href="{{ route('admin.zodiac-signs.create') }}" class="btn btn-success float-end">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody id="sortable">
                                        @foreach ($zodiacSigns as $index => $item)
                                            <tr data-id="{{ $item->id }}">
                                                <th scope="row">
                                                ...
                                                <td>{{ $item->slug }}</td>
                                                <td>
                                                    <a href="{{ route('admin.zodiac-signs.edit', $item->slug) }}"
                                                        class="btn btn-primary">
                                                        <i class="fa-solid fa-pen-to-square"></i>
                                                    </a>
                                                    <a class="btn btn-danger delete-confirm"
                                                        data-url="{{ route('admin.zodiac-signs.delete', $item->slug) }}">
        ...
        $(function() {
            $('#sortable').sortable({
                update: function(event, ui) {
                    ...
                    $.ajax({
                        url: '{{ route('admin.zodiac-signs.reorder') }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            order: order
